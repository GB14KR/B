...
def zeros(*shape):
    ...
    return Param(shape, lambda key: jnp.zeros(shape))

def ones(*shape):
    shape = jax.core.canonicalize_shape(shape)
    return Param(shape, lambda key: jnp.ones(shape))

def normal(*shape, stddev=1.0):
    shape = jax.core.canonicalize_shape(shape)
    return Param(shape, lambda key: stddev * jax.random.normal(key, shape))

def const(tensor):
    shape = jax.core.canonicalize_shape(tensor.shape)
    return Param(shape, lambda key: tensor)
...
def uniform(*shape, min=-1.0, max=1.0):
    shape = jax.core.canonicalize_shape(shape)
    return Param(shape, lambda key: jax.random.uniform(key, shape, minval=min, maxval=max))
